...
def _p(name, category, cities, bio, avatar, cover, portfolio, services, price_min, price_max, rating, reviews, phone, ig, verified=True):
    return {
        "id": str(uuid.uuid4()),
        "owner_id": None,
        "name": name,
...
SEED_PROVIDERS = [
    _p("Aanya Kapoor MUA", "Makeup Artists", ["Delhi", "Gurgaon", "Noida"],
       "Bridal & editorial makeup artist with 8+ years crafting timeless looks for Delhi brides.",
       AVATARS_W[0], MAKEUP_IMGS[0], MAKEUP_IMGS[:6],
    ...
    _p("Sunehri Cakes by Riya", "Bakers", ["Delhi", "Gurgaon"],
       "Boutique baker specializing in fondant tier cakes and dessert tables for weddings.",
       AVATARS_W[1], BAKER_IMGS[0], BAKER_IMGS[:6],
    ...
    _p("Mandap Magic Decor", "Decorators", ["Delhi", "Noida", "Gurgaon", "Ghaziabad"],
       "Full-service wedding decor — mandaps, floral installations and stage design.",
       AVATARS_M[0], DECOR_IMGS[0], DECOR_IMGS[:6],
    ...
    _p("Frame & Feel Studios", "Photographers", ["Mumbai", "Pune", "Goa"],
       "Candid wedding photography duo capturing real emotions in cinematic style.",
       AVATARS_M[1], PHOTO_IMGS[0], PHOTO_IMGS[:6],
    ...
    _p("Glow by Meher", "Makeup Artists", ["Mumbai", "Pune"],
       "Airbrush makeup specialist for the camera — featured in Vogue India weddings.",
       AVATARS_W[2], MAKEUP_IMGS[1], MAKEUP_IMGS[:6],
    ...
    _p("Floraison Decor", "Decorators", ["Bangalore", "Mysore"],
       "Modern minimal floral decor for intimate weddings and brunch parties.",
       AVATARS_W[3], DECOR_IMGS[1], DECOR_IMGS[:6],
    ...
    _p("Crumb Boutique", "Bakers", ["Bangalore"],
       ...
       AVATARS_W[0], BAKER_IMGS[1], BAKER_IMGS[:6],
    ...
    _p("Pixel Diaries", "Photographers", ["Bangalore", "Chennai", "Coimbatore"],
       "South Indian wedding specialists with a candid storytelling approach.",
       AVATARS_M[3], PHOTO_IMGS[1], PHOTO_IMGS[:6],
    ...
    _p("Plan With Pari", "Event Planners", ["Delhi", "Jaipur", "Udaipur", "Goa"],
       "End-to-end wedding planner for couples who want it stress-free and Pinterest-perfect.",
       AVATARS_W[1], DECOR_IMGS[2], DECOR_IMGS[:6],
    ...
    _p("Tandoor Tales Catering", "Caterers", ["Delhi", "Noida", "Gurgaon"],
       "Authentic Awadhi & Mughlai catering for cocktail & sangeet evenings.",
       AVATARS_M[0], CATER_IMGS[1], CATER_IMGS[:6],
    ...
    _p("Hyderabad Henna Studio", "Makeup Artists", ["Hyderabad", "Secunderabad"],
       "Bridal mehendi + makeup combo packages — designs inspired by Hyderabadi nizami art.",
       AVATARS_W[2], MAKEUP_IMGS[2], MAKEUP_IMGS[:6],
    ...
    _p("Pune Petal Decor", "Decorators", ["Pune", "Mumbai"],
       "Bohemian & rustic decor specialists — perfect for vineyard and outdoor weddings.",
       AVATARS_W[3], DECOR_IMGS[3], DECOR_IMGS[:6],
    ...
    _p("Mehfil Events Jaipur", "Event Planners", ["Jaipur", "Udaipur", "Jodhpur", "Pushkar"],
       "Royal Rajasthani destination weddings — palace bookings, decor and logistics.",
       AVATARS_M[1], DECOR_IMGS[4], DECOR_IMGS[:6],
    ...
    _p("Chennai Click Co.", "Photographers", ["Chennai", "Pondicherry"],
